...
<input type="Email" {...register("email",{required:"Email is required"})} class="border w-full h-10 px-3 mb-5 rounded-md" placeholder="Username or Email"/>
<p className='text-red-600'>{errors.password?.message}</p>
<label for="" class="block">Password</label>
<input type="password" {...register("password",{required:"password is required"})} class="border w-full h-10 px-3 mb-5 rounded-md" placeholder="password"/>
       <p className='text-red-600'>{errors.password?.message}</p>
       {error && (  <p className='text-red-600'>{error}</p>)}
       <p className='text-black'>Dont have an account ?  <Link to="/signup" className='text-green-600'> Register</Link> </p>
      
<button class="mt-5 bg-green-500 hover:bg-green-600 shadow-xl text-white uppercase text-sm font-semibold px-14 py-3 rounded"> 
