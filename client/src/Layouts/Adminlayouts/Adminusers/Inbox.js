...
import { Adminusersactions } from '../../../Store/Store'
import Adminsauth from '../../../Auth/Adminsauth/Adminsauth'
function AdminInbox() {
   const dispatch=useDispatch()
   const data=useLoaderData()
    const {status,Warning,products}=useSelector((state)=>state.Adminusersmanagement)
    const {ProductApproval}=Adminsauth()
    useEffect(() => {
        dispatch(Adminaction.products(data))
    }, [])

    const Adminapproved=products?.filter((product)=>!product.isAdminapproved)
    
     const Approval=(id)=>{
      dispatch(ProductApproval(id))
     }
  return (
   <AdminusersLayout>
...
{Adminapproved?.length===0 && (<div class="flex justify-evenly">
<h1 class="ml-4 text-2xl text-center font-bold text-gray-800">No approvals pending</h1>
</div>
)}

{Adminapproved && (<section class="py-10 ">
  <div class="mx-auto grid max-w-6xl  grid-cols-1 gap-6 p-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
  {Adminapproved?.map((product)=>{
        ...
        <div class="mt-1 p-2">
          ...
          <div class="mt-3 flex items-end justify-between">
            ...
            <div class="flex items-center space-x-1.5 rounded-lg bg-green-500 px-4 py-1.5 text-white duration-100 hover:bg-blue-600">
              ...
              <button class="text-sm " onClick={()=>Approval(product._id)}>Approve</button>
            </div>
          </div>
        </div>
      </a>
