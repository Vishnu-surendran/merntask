...
function Adminlogin() {
  ...
  const {Admlogin}=Adminsauth()
  const {isloading,error}=useSelector((state)=>state.Adminmanagement)
  const submit=async(data)=>{
    dispatch(Admlogin(data))
  }
  return (
   <>
   <Adminformlayout>
   <span class="border text-4xl text-center text-white px-6 pt-10 pb-8 bg-black w-1/2 max-w-md mx-auto rounded-t-md sm:px-10">Admin</span>
  <div class="border relative px-4 pt-7 pb-8 bg-black shadow-xl w-1/2 max-w-md mx-auto sm:px-10 rounded-b-md">
  <form onSubmit={handleSubmit(submit)}>
<label for="" class="block text-white">Username</label>
