...
import {UserIcon,HandRaisedIcon,ComputerDesktopIcon} from "@heroicons/react/24/solid"
import { NavLink,Link } from 'react-router-dom'
import { Adminaction, Adminusersactions } from '../../../Store/Store'
import Adminsauth from '../../../Auth/Adminsauth/Adminsauth'
function Adminuserslayout({children}) {
   const dispatch=useDispatch()
   const{Adminuserlogout}=Adminsauth()
    const {status}=useSelector((state)=>state.Adminusersmanagement)
    const Sidebar=()=>{
dispatch(Adminusersactions.statusChange(!status))
    }
              ...
              <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                 <div className="flex-1 px-3 bg-white divide-y space-y-1">
                    <ul className="space-y-8 pb-2">
                       ...
                       <li>
                       <NavLink to="/adminuser" className={({isActive})=> isActive ? " hidden text-base text-gray-900 font-normal rounded-l bg-gray-400 lg:flex items-center p-2 group ":" hidden text-base text-gray-900 font-normal rounded-lg  lg:flex items-center p-2 group " }>
                         
                         <span className="ml-2 flex-1 whitespace-nowrap font-semibold">Dashboard</span>
                      </NavLink>
                      <NavLink to="/adminuser" className={({isActive})=> isActive ? "bg-gray-400" :undefined} >
                     <ComputerDesktopIcon className='ml-2 w-8 lg:hidden'/>
                     </NavLink>
                       </li>
                       <li >
                          <NavLink to="/adminuser/products" className={({isActive})=> isActive ? " hidden text-base text-gray-900 font-normal rounded-l bg-gray-400 lg:flex items-center p-2 group ":" hidden text-base text-gray-900 font-normal rounded-lg  lg:flex items-center p-2 group " }>
                         
                             <span className="ml-2 flex-1 whitespace-nowrap font-semibold">Products</span>
                          </NavLink>
                          <NavLink to="/adminuser/products" className={({isActive})=> isActive ? "bg-gray-400" :undefined} >
                        <UserIcon className=' ml-2 w-8 lg:hidden'/>
                        </NavLink>
                       </li>
                       <li>

                          <NavLink to="/adminuser/inbox" className={({isActive})=> isActive ? "bg-gray-400" :undefined} >
                          <HandRaisedIcon onClick={logout} className='ml-2 w-8 lg:hidden'/>
                          </NavLink>
