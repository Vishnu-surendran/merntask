...
const dispatch=useDispatch()
    ...
    const{Adminlogout}=Adminsauth()
    const Sidebar=()=>{
      dispatch(Adminaction.statusChange(!status))
          }
const logout=()=>{
 Adminlogout()
       ...
       <div className="px-3 py-3 lg:px-5 lg:pl-3">
          <div className="flex items-center justify-between">
             ...
             <div className="flex items-center">
                ...
                <Link onClick={logout} replace className="hidden sm:inline-flex ml-5 text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center items-center mr-3">
                   Logout
                </Link>
             </div>
          </div>
             ...
             <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                <div className="flex-1 px-3 bg-white divide-y space-y-1">
                   <ul className="space-y-8 pb-2">
                      ...
                      <li>
                      <NavLink to="/admin" className={({isActive})=> isActive ? " hidden text-base text-gray-900 font-normal rounded-l bg-gray-400 lg:flex items-center p-2 group ":" hidden text-base text-gray-900 font-normal rounded-lg  lg:flex items-center p-2 group " }>
                        
                        <span className="ml-2 flex-1 whitespace-nowrap font-semibold">Dashboard</span>
                     </NavLink>
                     <NavLink to="/admin" className={({isActive})=> isActive ? "bg-gray-400" :undefined} end >
                    <ComputerDesktopIcon className='ml-2 w-8 lg:hidden'/>
                    </NavLink>
                      </li>
                      <li >
                         <NavLink to="/admin/users" className={({isActive})=> isActive ? " hidden text-base text-gray-900 font-normal rounded-l bg-gray-400 lg:flex items-center p-2 group ":" hidden text-base text-gray-900 font-normal rounded-lg  lg:flex items-center p-2 group " }>
                        
                            <span className="ml-2 flex-1 whitespace-nowrap font-semibold">AdminUsers</span>
                         </NavLink>
                         <NavLink to="/admin/users" className={({isActive})=> isActive ? "bg-gray-400" :undefined} >
                       <UserIcon className=' ml-2 w-8 lg:hidden'/>
                       </NavLink>
                      </li>
                      <li>
                         <NavLink to="/admin/inbox" className={({isActive})=> isActive ? " hidden text-base text-gray-900 font-normal rounded-l bg-gray-400 lg:flex items-center p-2 group ":" hidden text-base text-gray-900 font-normal rounded-lg  lg:flex items-center p-2 group " } end >
                           
                            <span className="ml-2 flex-1 whitespace-nowrap font-semibold">Inbox</span>
                         
                         </NavLink>
                         <NavLink to="inbox" className={({isActive})=> isActive ? "bg-gray-400" :undefined} >
                         <EnvelopeIcon className='ml-2 w-8 lg:hidden'/>
                         </NavLink>
                    <li><HandRaisedIcon onClick={logout} className='ml-2 mt-6 w-8 lg:hidden'/></li>
                      </li>                    
                   </ul>
